...
export default function Timeline({ items }: TimelineProps) {
  ...
  return (
    <div className="relative py-8">
      {/* Timeline line */}
      <div className="hidden md:block absolute left-6 top-0 bottom-0 w-0.5 bg-gradient-to-b from-cyan-400 to-cyan-600 opacity-30"></div>

      {items.map((item, index) => (
        <div
          key={index}
          className={`relative mb-12 md:pl-16 group ${item.slug ? 'cursor-pointer' : ''
            }`}
          onClick={() => handleItemClick(item)}
        >
          {/* Timeline marker */}
          <div className="hidden md:block absolute left-3 top-2">
            <div className={`w-6 h-6 rounded-full border-3 ${item.type === 'personal'
              ...
              } flex items-center justify-center ${item.slug ? 'group-hover:animate-pulse' : ''
              }`}>
              {item.type === 'personal' && (
          ...
          {/* Timeline content */}
          <div className={`bg-gray-800 border rounded-xl p-6 transition-all duration-300 ${item.type === 'personal'
            ...
            <div className="flex justify-between items-start mb-4 flex-wrap gap-4 md:flex-nowrap">
              <div className="flex items-center gap-4">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold ${item.type === 'personal'
                ...
                <div>
                  ...
                  <p className={`font-medium ${item.type === 'personal' ? 'text-purple-400' : 'text-cyan-400'
                    }`}>{item.company}</p>
                </div>
              </div>
            ...
            {item.slug && (
              <div className="mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className={`text-sm font-medium flex items-center gap-2 ${item.type === 'personal' ? 'text-purple-400' : 'text-cyan-400'
                  }`}>
